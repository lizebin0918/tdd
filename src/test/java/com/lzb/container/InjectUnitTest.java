...
public class InjectUnitTest extends BaseUnitTest {
    ...
    public void beforeEach() throws NoSuchFieldException {
        ...
        lenient().when(context.getType(eq(Dependency.class))).thenReturn(Optional.of(dependency));
        lenient().when(context.getType(eq(dependencyProviderType))).thenReturn(Optional.of(dependencyProvider));
    }
