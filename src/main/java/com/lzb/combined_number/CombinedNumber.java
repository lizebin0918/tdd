...
public class CombinedNumber {
    ...
    public static String combine(int[] inputs) {
        int maxLength = IntStream.of(inputs).boxed().map(Objects::toString).max(Comparator.comparing(item -> item.length())).map(String::length).orElse(0);
        for (int j = 0; j < maxLength; j++) {
            for (int i = 0; i < inputs.length; i++) {
                int maxIndex = maxNumOfLeftMost(inputs, i, 0);
                if (maxIndex != i) {
